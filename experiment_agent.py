#!/usr/bin/env python3
"""
implemented by me, inspired by week 6 workshop on Agentic AI
...
**Policy default: Hugging Face** — loads a small instruct model with `transformers` (no separate server):
  ...
  --policy hf         — local causal LM from the Hub (default: gemma-4-4B-it).
  --hf-model gemma-4-4B-it  — override; or set EXPERIMENT_AGENT_HF_MODEL.
"""
...
#Created so the file can run as a script with proper arg parsing, this is the main entry point of the code
#Due to compute constraints, the code implements a checkpointing system to save progress and also a start, stop, continue functionality
#This is useful for long running experiments that may be interrupted
#The code is also modular and can be easily extended to support new experiments
def main():
    args = parse_args()
